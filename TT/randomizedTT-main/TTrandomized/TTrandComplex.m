function X = TTrandComplex(I, R)
% TTRANDComplex compute a complex random TT-tensor with normalized independent Gaussian-distributed entries.
%
...
X = cell(N, 1);
for n = 1 : N
    X{n} = randn(I(n) * R(n), R(n + 1)) + 1i*randn(I(n) * R(n), R(n + 1));
